...
/**
 * An action that uses two PID controllers ({@link PIDController}) and a
 * ProfiledPIDController
 * ({@link ProfiledPIDController}) to follow a trajectory {@link Trajectory}
 * with a swerve drive.
 *
 * <p>
 * This action outputs the raw desired Swerve Module States
...
public class SwerveTrajectoryAction implements Action {
    ...
    /**
     * Constructs a new SwerveTrajectoryAction that when executed will follow the
     * provided
    ...
    /**
     * Constructs a new SwerveControllerAction that when executed will follow the
     * provided
     * trajectory. This command will not return output voltages but rather raw
     * module states from the
     * position controllers which need to be put into a velocity PID.
    ...
    public void update() {
        ...
        if (m_wantsVisionAlign.get()) {
            if (mLimelight.hasTarget()) {
                desiredRotation = Rotation2d
                        .fromDegrees(m_pose.get().getRotation().getDegrees() - mLimelight.getOffset()[0]);
            } else {
                desiredRotation = Rotation2d.fromDegrees(Superstructure.getInstance().getRealAimingParameters().get()
                        .getVehicleToGoalRotation().getWPIRotation2d().getDegrees() + 180.0);
                System.out.println(Superstructure.getInstance().getRealAimingParameters().get()
                        .getVehicleToGoalRotation().getWPIRotation2d().getDegrees() + 180.0);
            }
        } else {
        ...
        var targetChassisSpeeds = m_controller.calculate(m_pose.get(), desiredState, desiredRotation);
    ...
    public void done() {
        m_timer.stop();
        m_outputModuleStates.accept(m_kinematics
                .toSwerveModuleStates((ChassisSpeeds.fromFieldRelativeSpeeds(0, 0, 0, Rotation2d.fromDegrees(0)))));
    }
